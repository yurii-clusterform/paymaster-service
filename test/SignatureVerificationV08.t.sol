...
import {SignatureVerifyingPaymasterV08} from "../contracts/SignatureVerifyingPaymasterV08.sol";
...
contract SignatureVerificationV08 is Test {
    ...
    SignatureVerifyingPaymasterV08 paymaster;
    ...
    function setUp() public {
        ...
        //deploy paymaster
        address implementation  = address(new SignatureVerifyingPaymasterV08(entryPoint));

        paymaster = SignatureVerifyingPaymasterV08(payable(Clones.clone(implementation)));
    ...
    function test_cant_reuse_paymaster_signature() public {
        ...
        PackedUserOperation
            memory unsignedUserOp = _generateUnsignedUserOperationV08(
                executeCallData,
                address(account),
                nonce,  
                signer
    ...
    function _generateUnsignedUserOperationV08(
        bytes memory callData,
        address sender,
        uint256 nonce,
        VmSafe.Wallet memory PaymasterSigner
        ...
        (uint8 v, bytes32 r, bytes32 s) = vm.sign(PaymasterSigner, typedDataHash);
    ...
    function _hashTypedDataV4(bytes32 structHash) internal view virtual returns (bytes32) {
        return MessageHashUtils.toTypedDataHash(buildDomainSeparator(), structHash);
    }

    function buildDomainSeparator() internal view returns (bytes32) {

        return paymaster.domainSeparator();
    }
